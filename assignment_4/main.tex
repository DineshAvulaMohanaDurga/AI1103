...
\begin{enumerate}
\item Option 1 defines P\brak{X} only in region 3 but not 1,2.\\
...
\end{align}
Here G\brak{x,y} is 0 still F is not continuous at \brak{x,y} parellel to X-axis
\begin{align}
F\brak{X,Y}&=1 \hspace{0.3in} X>x,Y>y_1 \nonumber\\
&=0 \hspace{0.3in} otherwise
\end{align}
\textbf{So this option is false}
\item Option 2 defines P\brak{X} only in region 3 and either region 1 or region 2.\\
If we take the same example in equation 2.0.6 it satisfies G\brak{Y=y}=0 i.e option 2 but F is not continuous at \brak{x,y}.\\
\textbf{So this option is also false}
\item Option 3 defines p\brak{X} in all three regions.\\
\textbf{So option 3 is true}
\item Option 4 also defines p\brak{X} in all three regions.\\
